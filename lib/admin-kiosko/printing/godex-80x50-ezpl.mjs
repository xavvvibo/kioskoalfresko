...
export function buildGodex80x50PrepProfessionalEzpl(input = {}) {
  ...
  const qrValue = safeQrText(input.qrValue || "", 240);
  const includeQr = Boolean(qrValue.startsWith("https://") && input.includeQr !== false);
  const qrGraphic = includeQr ? nativeQrCommand(qrValue, { x: 360, y: 150, model: 3, multiplier: 2, errorCorrection: "M", mask: 8, rotation: 5, max: 240 }) : "";
