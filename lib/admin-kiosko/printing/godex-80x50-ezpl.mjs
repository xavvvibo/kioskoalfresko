...
export function buildGodex80x50AppccTraceabilityEzpl(input = {}) {
  ...
  const state = safeText(input.state || "TRAZABILIDAD", 24);
  ...
  const purchaseDate = safeText(input.purchaseDate || "", 14);
  const purchaseDateLabel = safeText(input.purchaseDateLabel || "COMPRA", 14);
  const freezingDate = safeText(input.freezingDate || "", 14);
  const originalExpiryDate = safeText(input.originalExpiryDate || "", 14);
  const defrostedAt = safeText(input.defrostedAt || "", 20);
  const eventDateLabel = safeText(input.eventDateLabel || "SALIDA", 14);
  const useBy = safeText(input.useBy || "", 20);
  const responsible = safeText(input.responsible || "", 24);
  const storage = safeText(input.storage || "", 34);
  const warning = safeText(input.warning || "", 34);
  const note = safeText(input.note || "", 44);
  const trace = safeQrText(input.traceValue || batch || product, 180);
  const qrGraphic = trace ? nativeQrCommand(trace, { x: 496, y: 56, model: 3, multiplier: 2, errorCorrection: "M", mask: 8, rotation: 5 }) : "";
  const rows = [
    quantity ? `CANT: ${quantity}` : "",
    supplier ? `PROV: ${supplier}` : "",
    ticket ? `FACT/REF: ${ticket}` : "",
    ...
    useBy ? `USAR ANTES: ${useBy}` : "",
    storage ? `CONSERV: ${storage}` : "",
    responsible ? `RESP: ${responsible}` : "",
  ].filter(Boolean);
  const rowCommands = rows.slice(0, 10).map((row, index) => visibleTextCommand(20, 104 + index * 24, row, { max: index < 2 ? 40 : 36 }));

  return buildEzpl([
    lineCommand(0, 0, 640, 0, 8),
    visibleTextCommand(18, 16, "KIOSKO ALFRESKO", { max: 20 }),
    visibleTextCommand(382, 16, state, { max: 24 }),
    lineCommand(0, 42, 640, 42, 3),
    visibleTextCommand(20, 58, product, { width: 2, height: 2, max: 24 }),
    qrGraphic,
    ...rowCommands,
    lineCommand(20, 348, 620, 348, 2),
    warning ? visibleTextCommand(20, 358, warning, { width: 2, height: 1, max: 30 }) : "",
    note ? visibleTextCommand(20, 382, note, { max: 44 }) : "",
    batch ? visibleTextCommand(350, 382, `LOTE ${batch}`, { max: 28 }) : "",
  ].filter(Boolean), { copies: input.copies });
}
