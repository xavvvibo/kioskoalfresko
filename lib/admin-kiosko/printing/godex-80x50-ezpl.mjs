...
export function generateGodexQrEzpl(x, y, value, options = {}) {
  const qrValue = safeText(value, options.max ?? 180);
  if (!qrValue) return "";

  // QR EZPL is isolated because GoDEX firmware revisions may differ.
  // If a firmware needs a variant, adjust only this function.
  const model = options.model ?? "M2";
  const errorCorrection = options.errorCorrection ?? "M";
  const cell = options.cell ?? 6;
  return `QRCODE,${x},${y},${errorCorrection},${cell},A,0,${model},S7,"${qrValue}"`;
}
...
export function buildGodex80x50PrepProfessionalEzpl(input = {}) {
  ...
  return buildEzpl([
    ...
    ...rowCommands,
    includeQr ? generateGodexQrEzpl(438, 124, qrValue, { cell: 6 }) : "",
    includeQr ? visibleTextCommand(438, 276, "QR INTERNO", { max: 16 }) : "",
