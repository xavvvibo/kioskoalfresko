      ...
      <div className="mt-8 rounded-[1.7rem] border border-stone-950/12 bg-stone-50 p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {seasonalSchedule
            ...
            .map((item) => (
              <article key={item.month} className="rounded-[1.3rem] border border-stone-950/10 bg-white p-4">
                <div className="flex items-center justify-between gap-3">
                  <h4 className="text-lg font-black uppercase tracking-[-0.03em] text-stone-950">{item.month}</h4>
                  <span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] ${statusClasses[item.status]}`}>
                    {item.status === "special" ? "Especial" : "Pendiente"}
                  </span>
                </div>
