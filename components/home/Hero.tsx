      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-4 pb-14 pt-10 sm:px-6 md:grid-cols-[1.08fr_0.92fr] md:gap-10 md:pb-24 md:pt-16">
        ...
        <div className="relative grid gap-4 self-end md:pl-6">
          ...
          <div className="relative overflow-hidden rounded-[2.4rem] border border-stone-950 bg-stone-950 p-7 text-white shadow-[0_34px_90px_rgba(0,0,0,0.22)] md:p-8">
            ...
            <p className="relative z-10 text-xs font-black uppercase tracking-[0.22em] text-[#efb7a8]">Temporada abierta</p>
            <p className="relative z-10 mt-5 text-[2.4rem] font-black uppercase leading-[0.9] tracking-[-0.06em] md:text-[3.8rem]">Arrancamos temporada en el Parque San Sebastián.</p>
            <p className="relative z-10 mt-5 max-w-sm text-sm leading-6 text-stone-300 md:text-base">Tarde, noche y plan fácil. El 25 de abril volvemos a abrir.</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-[1.8rem] border border-stone-950 bg-white p-5 shadow-[0_12px_28px_rgba(0,0,0,0.06)]">
              <div className="text-[11px] font-black uppercase tracking-[0.2em] text-[#d94b2b]">Reapertura</div>
              <div className="mt-3 text-lg font-black uppercase leading-tight text-stone-950 md:text-[1.35rem]">{siteConfig.schedule.currentSummary}</div>
            </div>
            <div className="rounded-[1.8rem] border border-stone-950 bg-[#d94b2b] p-5 text-white shadow-[0_16px_34px_rgba(217,75,43,0.22)]">
              <div className="text-[11px] font-black uppercase tracking-[0.2em] text-white/80">Dónde</div>
              <div className="mt-3 text-lg font-black uppercase leading-tight md:text-[1.35rem]">{siteConfig.location.area}<br />{siteConfig.location.city}</div>
            </div>
          </div>
        </div>
      </div>
