...
export default async function StaffOnboardingPage() {
  const session = await requireAdminSession("/staff/incorporacion");
  ...
  const [processes, tasks] = await Promise.all([listStaffProcesses("onboarding", employee.data.id), listProcessTasks(undefined, employee.data.id)]);
