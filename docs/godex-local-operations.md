...
arp -an | grep '192.168.1.37' || true
ping -c 3 192.168.1.37 || true
nc -G 3 -vz 192.168.1.37 80 || true
nc -G 3 -vz 192.168.1.37 515 || true
nc -G 3 -vz 192.168.1.37 9100 || true
curl --max-time 5 http://192.168.1.37 || true
```
...
El healthcheck queda en `http://127.0.0.1:8787/health`. El bridge debe ejecutarse en un equipo de la misma red que la impresora. Vercel no debe intentar abrir `192.168.1.37`.
...
1. Actualizar `GODEX_PRINTER_HOST` en `/Users/xavibocanegra/kioskoalfresko/.env.local`.
...
- Confirmar que `ping -c 3 192.168.1.37` responde.
- Confirmar `nc -G 3 -vz 192.168.1.37 9100`.
